 ...
 *
 * This one implements in-kernel interface used in smbsrv, and
 * calls the user-level idmap API for it's "back-end" support.
 * There are longer comments in the uts variant.  The code in
 * this variant looks a lot like the one in libsmb.
 */
