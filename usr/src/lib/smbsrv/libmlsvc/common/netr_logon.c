...
netlogon_logon(smb_logon_t *user_info, smb_token_t *token, smb_domainex_t *di)
{
	...
	uint32_t status;
	boolean_t did_reauth = B_FALSE, did_renego = B_FALSE;
...
reauth:
	if ((netr_global_info.flags & NETR_FLG_VALID) == 0 ||
		 ...
		 */
		status = netlogon_auth(di->d_dci.dc_name,
		    di->d_primary.di_nbname, NETR_FLG_NULL);

		if (status != 0) {
			syslog(LOG_ERR, "netlogon_logon: auth failed (%s)",
	 ...
	 */
	status = netr_open_secure(di->d_dci.dc_name, di->d_primary.di_nbname,
	    &netr_handle);
	if (status != 0) {
		/*
		...
		if (!did_renego) {
			did_renego = B_TRUE;
			netr_invalidate_chain();
			syslog(LOG_ERR, "netlogon_logon: open failed (%s); "
			    "renegotiating...",
			    xlate_nt_status(status));
			goto reauth;
		}
...
reauth:
	if ((netr_global_info.flags & NETR_FLG_VALID) == 0 ||
		...
		if (status != 0) {
			syslog(LOG_ERR, "netlogon_check: auth failed (%s)",
			    xlate_nt_status(status));
			goto unlock_out;
		} else {
			netr_global_info.flags |= NETR_FLG_VALID;
	...
	status = netr_open_secure(server, domain,
	    &netr_handle);
	if (status != 0) {
		/*
		 * This may have failed because the DC restarted.
		...
		if (!did_renego) {
			did_renego = B_TRUE;
			netr_invalidate_chain();
			syslog(LOG_ERR, "netlogon_check: open failed (%s); "
			    "renegotiating...",
			    xlate_nt_status(status));
			goto reauth;
		}
    ...
    struct netr_authenticator *auth, struct netr_authenticator *ret_auth)
{
	...
	/*
	 * Windows DCs will reject Authenticators if the first
	 * 5 bytes of the ClientStoredCredential are all the same.
