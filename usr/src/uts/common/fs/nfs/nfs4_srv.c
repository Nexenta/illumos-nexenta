...
rfs4_grace_start(nfs_server_instance_t *sip)
{
	...
	sip->gstart_time = (time_t)TICK_TO_SEC(ddi_get_lbolt());
    ...
    struct compound_state *cs)
{
	 ...
	 */
	va.va_mask = AT_MODE;
	error = nfs_vop_getattr(vp, &va, 0, cr, NULL, cs->exi);
	if (error) {
    ...
    struct compound_state *cs)
{
	...
	error = nfs_vop_getattr(dvp, &bva, 0, cr, NULL, cs->exi);
	if (error) {
		*cs->statusp = resp->status = puterrno4(error);
		if (name != nm)
			kmem_free(name, MAXPATHLEN + 1);
	...
	case NF4DIR:
		 ...
		 */
		iva.va_mask = AT_SEQ;
		if (nfs_vop_getattr(dvp, &iva, 0, cs->cr, NULL, cs->exi))
			iva.va_seq = 0;
		break;
	case NF4LNK:
		vap->va_type = VLNK;
		 ...
		 */
		iva2.va_mask = AT_SEQ;
		if (nfs_vop_getattr(dvp, &iva2, 0, cs->cr, NULL, cs->exi) ||
	...
	if (nfs_vop_getattr(dvp, &ava, 0, cr, NULL, cs->exi)) {
 ...
 * It calls both nfs_vop_getattr and VFS_STATVFS calls to get the attrs.
 * Returns nfsv4 status.
 */
nfsstat4
bitmap4_get_sysattrs(struct nfs4_svgetit_arg *sargp)
{
	...
	return (rfs4_vop_getattr(vp, sargp->vap, 0, cs->cr, cs->exi));
}
    ...
    struct nfs4_svgetit_arg *sargp)
{
	...
	ASSERT(nfs4_attrvers(sargp->cs) == ntovp->attrvers);
	...
	if (status == NFS4_OK) {
		...
		if (status == NFS4_OK) {
			...
			if (vn_is_nfs_reparse(cs->vp, cs->cr)) {
				...
				if (client_is_downrev(cs->instp, req))
    ...
    struct compound_state *cs)
{
	...
	error = nfs_vop_getattr(dvp, &bdva, 0, cs->cr, NULL, cs->exi);
	if (error) {
		*cs->statusp = resp->status = puterrno4(error);
		kmem_free(nm, len);
		goto out;
	 ...
	 */
	idva.va_mask = AT_SEQ;
	if (nfs_vop_getattr(dvp, &idva, 0, cs->cr, NULL, cs->exi))
		idva.va_seq = 0;
	...
	if (nfs_vop_getattr(dvp, &adva, 0, cs->cr, NULL, cs->exi)) {
    ...
    struct compound_state *cs)
{
	...
	va.va_mask = AT_MODE;
	error = nfs_vop_getattr(vp, &va, 0, cs->cr, NULL, cs->exi);
	if (error) {
		*cs->statusp = resp->status = puterrno4(error);
		goto out;
	}
    ...
    struct compound_state *cs)
{
	...
	if (vp->v_type == VDIR) {
		 ...
		 */
		va.va_mask = AT_NLINK;
		if (!nfs_vop_getattr(vp, &va, 0, cs->cr, NULL, cs->exi) &&
		    va.va_nlink == 0) {
    ...
    struct compound_state *cs)
{
	...
	error = nfs_vop_getattr(odvp, &obdva, 0, cs->cr, NULL, cs->saved_exi);
	if (!error) {
		nbdva.va_mask = AT_CTIME|AT_SEQ;
		error = nfs_vop_getattr(ndvp, &nbdva, 0, cs->cr, NULL, cs->exi);
	}
	if (error) {
		*cs->statusp = resp->status = puterrno4(error);
		goto err_out;
	...
	if ((error = VOP_RENAME(odvp, converted_onm, ndvp, converted_nnm,
		...
		if (tvp) {
			va.va_mask = AT_NLINK;
			if (!nfs_vop_getattr(tvp, &va, 0, cs->cr, NULL,
				cs->exi) &&
			    va.va_nlink == 0) {
	...
	if (nfs_vop_getattr(odvp, &oidva, 0, cs->cr, NULL, cs->saved_exi))
		oidva.va_seq = 0;

	nidva.va_mask = AT_SEQ;
	if (nfs_vop_getattr(ndvp, &nidva, 0, cs->cr, NULL, cs->exi))
		nidva.va_seq = 0;
	...
	if (nfs_vop_getattr(odvp, &oadva, 0, cs->cr, NULL, cs->saved_exi)) {
		oadva.va_ctime = obdva.va_ctime;
		oadva.va_seq = 0;
	}

	nadva.va_mask = AT_CTIME|AT_SEQ;
	if (nfs_vop_getattr(ndvp, &nadva, 0, cs->cr, NULL, cs->exi)) {
	...
	attrmap4 *resp, struct nfs4_ntov_table *ntovp)
{
	...
	if (sva_mask != 0) {
		 ...
		 */
		ret_error = nfs_vop_getattr(sargp->cs->vp, sargp->vap, 0,
		    sargp->cs->cr, NULL, sargp->cs->exi);
		if (ret_error) {
			if (resp == NULL)
				return (ret_error);
			/*
	...
	ASSERT(ntovp->attrvers == nfs4_attrvers(sargp->cs));
    ...
    nfs4_attr_cmd_t cmd)
{
	...
	ASSERT(ntovp->attrvers == nfs4_attrvers(cs));
	avers = ntovp->attrvers;
	nvmap = NFS4_NTOV_MAP(avers);
	nvmap_size = NFS4_NTOV_MAP_SIZE(avers);
    ...
    stateid4 *stateid)
{
	...
	if (vp->v_type == VREG && (sarg.vap->va_mask & AT_SIZE)) {
		...
		if (error = nfs_vop_getattr(vp, &bva, 0, cr, &ct, cs->exi)) {
			status = puterrno4(error);
			goto done;
		}
		...
		if (crgetuid(cr) == bva.va_uid) {
			off64_t offset = sarg.vap->va_size;

			if (pnfs_enabled(cs->exi))
				offset = pnfs_shadow_size(sarg.vap->va_size);
	...
	if (!error && sarg.vap->va_mask != 0)
		error = nfs_vop_setattr(vp, sarg.vap, sarg.flag, cr, &ct,
		    cs->exi);
    ...
    struct compound_state *cs, uint32_t access, change_info4 *cinfo)
{
	...
	error = nfs_vop_getattr(dvp, &bva, 0, cs->cr, NULL, cs->exi);
	if (error)
		return (puterrno4(error));
	 ...
	 */
	ava.va_mask = AT_CTIME|AT_SEQ;
	if (nfs_vop_getattr(dvp, &ava, 0, cs->cr, NULL, cs->exi)) {
		ava.va_ctime = bva.va_ctime;
		ava.va_seq = 0;
	}
	VN_RELE(dvp);
	 ...
	 * cs->vp and cs->exi were updated
	 */
	fva.va_mask = AT_TYPE|AT_MODE;
	error = nfs_vop_getattr(cs->vp, &fva, 0, cs->cr, NULL, cs->exi);
	if (error)
		return (puterrno4(error));
	...
	*created = TRUE;
	...
	if (error) {
		*created = FALSE;
		...
		ASSERT(mtime != 0);
		va.va_mask = AT_MTIME;
		error = nfs_vop_getattr(*vpp, &va, 0, cr, NULL, exi);
		if (!error) {
			/* We found the file */
    ...
    change_info4 *cinfo, attrmap4 *attrset, clientid4 clientid)
{
	...
	if (nfs_vop_getattr(vp, &cva, 0, cs->cr, NULL, cs->exi) == 0) {
		...
		if (trunc) {
			...
			cva.va_mask = AT_SIZE;
			cva.va_size = reqsize;
			(void) nfs_vop_setattr(vp, &cva, 0, cs->cr, &ct,
			    cs->exi);
			if (in_crit)
				nbl_end_crit(vp);
		}
    ...
    OPEN4args *args, rfs4_openowner_t *oo, OPEN4res *resp)
{
	...
	if (resp->status == NFS4_OK) {
		 ...
		 */
		if (resp->status != NFS4_OK) {
			*attrset = NFS4_EMPTY_ATTRMAP(nfs4_attrvers(cs));
		}
	}
	else
		*cs->statusp = resp->status;
    ...
    OPEN4args *args, rfs4_openowner_t *oo, OPEN4res *resp)
{
	...
	error = nfs_vop_getattr(cs->vp, &va, 0, cs->cr, NULL, cs->exi);
	if (error) {
		resp->status = puterrno4(error);
		return;
	}
...
retry:
	...
	if (strstr(args->client.id_val, "Solaris") &&
		...
		create = TRUE;
		ci = rfs4_find_clntip(cs->instp, args->client.cl_addr, &create);
		ASSERT(ci != NULL);
		rfs4_dbe_rele(ci->ri_dbe);
	} else {
		/* Remove any previous record */
		rfs4_invalidate_clntip(cs->instp, args->client.cl_addr);
	}
 ...
 */
int
client_is_downrev(nfs_server_instance_t *instp, struct svc_req *req)
{
	struct sockaddr *ca;
	rfs4_clntip_t *ci;
	bool_t create = FALSE;
	...
	ca = (struct sockaddr *)svc_getrpccaller(req->rq_xprt)->buf;
	ASSERT(ca);
	ci = rfs4_find_clntip(instp, ca, &create);
	if (ci == NULL)
		return (0);
